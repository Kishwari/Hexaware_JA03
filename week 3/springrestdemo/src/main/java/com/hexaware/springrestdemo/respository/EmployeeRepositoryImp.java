...
public class EmployeeRepositoryImp implements IEmployeeRepository {
	...
	public Employee updateEmployee(Employee employee) {
		Employee emp = null;
		String updateEmp = "update employee_table set ename = ? , salary = ? where eid = ?";
		int count = jdbcTemplate.update(updateEmp, employee.getEname(), employee.getSalary(), employee.getEid());
		if (count > 0)
			emp = employee;
		return emp;
	}
